...
class MatrixAnimation extends StatelessWidget {
  const MatrixAnimation({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return SingleChildScrollView(
      padding: EdgeInsets.only(left: 30, top: 70, bottom: 10, right: 10),
      child: Column(
...
class MatrixAnimationLayer extends StatelessWidget {
  const MatrixAnimationLayer({Key? key}) : super(key: key);
...
class CodeAnimation extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return DefaultTextStyle(
      ...
      child: AnimatedTextKit(
        repeatForever: true,
        animatedTexts: getBlocDatacode(13, dataCode)
  ...
  List<String> getBlocDatacode(int numRow, List<String> dataCode) {
    List<String> data = [];

    String tmp = '';
    int i = 0;

    for (var row in dataCode) {
      if (i < numRow) {
        tmp += "\n" + row;
        i++;
      } else {
